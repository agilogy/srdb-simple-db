...
class DslTest extends FlatSpec {
  ...
  it should "build simple select queries" in {
    val d = Departments("d")
    val selectDepts = from(d).where(d.id ==== 3l).select(d.*)(d.reads)
  ...
  it should "build join queries using a predicate" in {
    ...
    val joinPeopleDepts = from(p.join(d, p.departmentId ==== d.id))
  ...
  it should "build join queries using the same table with 2 different aliases" in {
    ...
    val s = from(c.join(m, c.motherFK)).select(c.* ++ m.*)(c.reads.join(m.reads))
  ...
  it should "build queries with order by clause" in {
    val (d, e) = (Departments("d"), Employees("e"))
    val s1 = from(e).orderBy(e.departmentId, e.name -> Desc).select(e.name)
    assert(s1.sql === "select e.name from employees e order by e.department_id, e.name desc")
    val s2 = from(e.join(d, e.departmentId ==== d.id)).orderBy(d.name, e.name -> Desc).select(e.name)
    assert(s2.sql === "select e.name from employees e join departments d on (e.department_id = d.id) order by d.name, e.name desc")
    val s3 = from(e).where(e.departmentId ==== 4l).orderBy(e.departmentId, e.name -> Desc).select(e.name)
    assert(s3.sql === "select e.name from employees e where (e.department_id = 4) order by e.department_id, e.name desc")
    val s4 = from(e).groupBy(e.departmentId, e.name).orderBy(e.departmentId, e.name).select(e.departmentId, e.name)
  ...
  it should "build an update statement" in {
    val d = Departments("d")
    val stmt = update(d).set(d.name := param(0))
  ...
  it should "build an insert statement" in {
    val d = Departments("d")
    val stmt = insertInto(d).values(d.id := param(0), d.name := param(1))
  ...
  they should "be identified to be passed as parameters when they are text" in {
    val d = Departments("d")
    val q = from(d).where(d.name ==== "--drop table employees" and d.name < "--boooh!").select(d.name)
  ...
  they should "be rewritten as their values when they are whitelisted text" in {
    val d = Departments("d")
    val q = from(d).where(d.name ==== "d1" and d.name < "abc").select(d.name)
